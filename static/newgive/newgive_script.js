      ...
      case "dates":
        ...
        if (giveawayData.startImmediately) {
          ...
          isValidDates =
            ...
            giveawayData.startDate !== "" &&
            parseDate(giveawayData.startDate) < parseDate(giveawayData.endDate);
        }
        return isValidDates;
      case "winners":
        return (
          giveawayData.winnersCount >= 1 && giveawayData.winnersCount <= 30
        );
      case "review":
        return true;
      default:
    ...
    switch (currentStepName) {
      ...
      case "dates":
        ...
        if (startImmediatelyToggle)
          giveawayData.startImmediately = startImmediatelyToggle.checked;
  ...
  function initStepSpecificLogic(step) {
    ...
    switch (currentStepName2) {
      case "basics":
        ...
        if (buttonTextInput)
          buttonTextInput.addEventListener("input", updateNextButtonState);
        if (selectPostBtn)
          selectPostBtn.addEventListener("click", updateNextButtonState);
      ...
      case "winners":
        const winnersCountInput = document.getElementById("winners-count");
        if (winnersCountInput)
          winnersCountInput.addEventListener("input", updateNextButtonState);
        break;
    }
    ...
    switch (type) {
      ...
      case "result":
        giveawayData.selectedResultChannels =
          giveawayData.selectedResultChannels.filter((id) => id !== channelId);
        break;
    }
  }
