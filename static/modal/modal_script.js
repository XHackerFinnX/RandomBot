...
async function updateChannel(channelId, userId) {
    try {
        const response = await fetch(
            `/api/update-channel?user_id=${userId}&channelid=${channelId}`,
            {
                method: "POST",
            }
        );
...
async function deleteChannel(channelId, userId) {
    try {
        const response = await fetch(
            `/api/delete-channel?user_id=${userId}&channelid=${channelId}`,
            {
                method: "POST",
            }
        );
...
async function deletePost(postId, userId) {
    try {
        const response = await fetch(
            `/api/delete-post?user_id=${userId}&postid=${postId}`,
            {
                method: "POST",
            }
        );
...
async function viewPost(postId, userId) {
    try {
        const response = await fetch(
            `/api/view-post?user_id=${userId}&postid=${postId}`,
            {
                method: "POST",
            }
        );
...
function createPostElement(post) {
  ...
  `;
    ...
    postElement
        ...
        .addEventListener("click", () => deletePost(post.id, post.user_id));
    postElement
        .querySelector(".view-button")
        .addEventListener("click", () => viewPost(post.id, post.user_id));
...
function createChannelElement(channel) {
    ...
    <div class="channel-actions">
      <button class="channel-button update-btn" data-id="${
          channel.id
      }" aria-label="Обновить">
      ...
      <button class="channel-button delete-btn" data-id="${
          channel.id
      }" aria-label="Удалить">
        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
          <polyline points="3 6 5 6 21 6"></polyline>
  ...
  `;
    ...
    if (updateBtn) {
        updateBtn.addEventListener("click", () =>
            updateChannel(channel.id, channel.user_id)
        );
    }
    ...
    if (deleteBtn) {
        deleteBtn.addEventListener("click", () =>
            deleteChannel(channel.id, channel.user_id)
        );
    }
...
async function loadChannelsToModal() {
    ...
    try {
        ...
        const channelListContainer = document.getElementById(
