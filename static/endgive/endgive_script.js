document.addEventListener("DOMContentLoaded", async function () {
    ...
    const raffleId = new URLSearchParams(window.location.search).get(
        "raffle_id"
    );
    ...
    const checkSubscriptionButton = document.getElementById(
    ...
    function populateChannels(channels) {
        ...
        channels.forEach((channel) => {
            ...
            const isSubscribed = channel.status_sub
                ? "subscribe-button button-sub"
                : "subscribe-button button-primary";
            ...
            <div class="flex items-center justify-between w-full">
                ...
                <div class="w-10 h-10 rounded-full overflow-hidden mr-3 flex-shrink-0">
                    ${
                        channel.photo_url
                            ...
                            : `<div class="w-full h-full flex items-center justify-center bg-primary text-white">${generateAvatarLetter(
                                  channel.name
                              )}</div>`
                    }
                </div>
                <div class="flex flex-col">
                    <h3 class="font-medium text-sm text-foreground">${
                        channel.name.length > maxLength
                            ? channel.name.slice(0, maxLength) + "..."
                            : channel.name
                    }</h3>
                </div>
                </div>
            ...
            const subscribeButton =
                channelElement.querySelector(".subscribe-button");
